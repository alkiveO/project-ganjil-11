...
export async function POST(req) {
  ...
  try {
    // Cek apakah pengajuan milik guru ini
    ...
    let pesan = "";
    if (status === "diterima") {
      pesan = `Pengajuan "${pengajuan.topik}" diterima oleh ${guru.name}`;
    } else if (status === "ditolak") {
      pesan = `Pengajuan "${pengajuan.topik}" ditolak oleh ${guru.name}`;
    } else if (status === "selesai") {
      pesan = `Konseling untuk topik "${pengajuan.topik}" telah selesai oleh ${guru.name}`;
    }
